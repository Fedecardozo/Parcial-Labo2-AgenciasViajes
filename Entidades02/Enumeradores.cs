
public enum EestadoViaje {En_Viaje, Disponible,Completo};

public enum Eregional
{
    Motevideo, Ushuaia, Recife, Santiago, Lima, Isla_de_Pascua, Isla_Galapagos,
    Puerto_Madryn, Rio_de_Janeiro, Cartagena
};

public enum EextraRegional { La_Habana, Venecia, Acapulco, Miami, Nueva_York, Bangkok, Taipei, Atenas };

public enum Eciudad { Regional, Extra_Regional };

public enum EtipoClase { Turistica, Premium };

public enum ENacionalidad { Argentina, Brasilera, Peruana, Paraguaya, Uruguaya, Española, Usa };

public enum ESexo { Femenino, Masculino, No_Binario };